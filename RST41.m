...
% taskInit = trvec2tform([0.2,0,0.6])*axang2tform([0 1 0 pi]);


% 定义目标位姿：位置偏移[0.2,0,0.2]，绕y轴旋转180度
taskFinal = trvec2tform([0.2,0,0.2])*axang2tform([0 1 0 pi]);
...
% 返回初始配置
show(robot,currentRobotJConfig,'PreservePlot',false,'Frames','off');
title(['机械臂轨迹规划演示 - 关节空间规划 (循环次数: ' num2str(currentLoop) ']'],'FontSize',14);
...
if ~getappdata(f, 'stopLoop')
        ...
        title(['机械臂轨迹规划演示 - 两种规划方式对比 (循环次数: ' num2str(currentLoop) ']'],'FontSize',14);
end
end
